...
export default function GrantSessionsScreen() {
  ...
  const handleLookup = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchSessionCredits(trimmed, 'rise');
  ...
  const handleWhatsApp = async () => {
    ...
    const message = `You have ${remaining} complimentary RISE Scheduler session${remaining === 1 ? '' : 's'} available. Book here: ${shareUrl}`;
