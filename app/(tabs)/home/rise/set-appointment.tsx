...
export default function SetAppointmentScreen() {
  ...
  const handleCreate = async () => {
    ...
    setCopied(false);
    try {
      const data = await createSchedulingInvite({ email: trimmed, type: inviteType, appSource: 'rise' });
  ...
  const handleWhatsApp = async () => {
    ...
    const message =
      invite.type === 'free'
        ? `You have a complimentary RISE session (100% off). Book here: ${invite.shareUrl}`
        : `Book your RISE session here: ${invite.shareUrl}`;
